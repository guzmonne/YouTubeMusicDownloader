...
var nodeYouTubeDownloader = {
		...
		if(fmt_map == ''){
			...
			for(var i in fmt_map){
				...
				if(ignoreFormats.indexOf(fmt_map[i].itag) == -1){
					...
					result.data.push({ itag: fmt_map[i].itag, url: fmt_map[i].url + "&signature=" + this.getSignature(fmt_map[i])});
	...
	downloadFileRealUrl: function(video){
		var options = {
		...
		http.get(options, function(res) {
			if([301, 302, 303, 307].indexOf(res.statusCode) > -1 && res.headers.location){
				...
				console.log("file size: " + (Math.round(parseInt(res.headers['content-length']) * 100.0 / (1024 * 1024)) / 100) + "MB.\n");
				...
				console.log("video is downloading and save as " + file_name + ", please wait...\n");
	...
	http.get(options, function(res) {
		if([301, 302, 303, 307].indexOf(res.statusCode) > -1 && res.headers.location){
			...
			res.on('data', function(data) {
				...
				process.stdout.write('Completed ' + (parseInt(file.bytesWritten) / file_size * 100) + '%');
			}).on('end', function() {
				file.end();
				process.stdout.clearLine();
				process.stdout.cursorTo(0);
...
function convertVideo(file_name, result_name){
	...
	ffmpeg(file_name)
		...
		.on('end', function(){
			process.stdout.clearLine();
			process.stdout.cursorTo(0);
			process.stdout.write('\n!!!\n' + result_name + '.mp3 converted successfully' + '\n!!!\n');
			fs.unlink(file_name);
		})
		.on('error', function(){
			process.stdout.clearLine();
			process.stdout.cursorTo(0);
			process.stdout.write('\n!!!\n' + 'Something heppend while converting ' + result_name + '.mp3' + '\n!!!\n');
		})
		.on('progress', function(info){
			process.stdout.clearLine();
			process.stdout.cursorTo(0);
			process.stdout.write('\n!!!\n' + result_name + '.mp3 ' + info.percent + '% done...' + '\n!!!\n');
		})
		.on('start', function(){
			process.stdout.clearLine();
			process.stdout.cursorTo(0);
			process.stdout.write('\n!!!\n' + 'Converting video ' + result_name + '.mp3 ' + '\n!!!\n');
		})
		.save(save_location);
}

function downloadNext(video){
	if (working){
		...
		if (queue.length === 0){
			if (video){
				...
				return process.stdout.write('No more files to download\n');
			}
		} else {
			download(queue.splice(0, 1)[0]);
		}
	}
}
...
function downloadFile(youtube_url, callback){
	var videos, checkResult = nodeYouTubeDownloader.validYouTubeUrl(youtube_url);
